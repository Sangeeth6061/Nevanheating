...
export default function ContactLocationMap({ locationMap }: ContactLocationMapProps) {
  return (
    <section className="w-full">
      <div
        className="relative w-full min-h-[280px] md:min-h-[320px] bg-[#D9CDB8] bg-cover bg-center"
        style={
          locationMap.mapImageUrl
            ? { backgroundImage: `url(${locationMap.mapImageUrl})` }
            : undefined
        }
      >
        <div className="absolute inset-0 flex items-center justify-center px-4 py-10">
          <div className="bg-white rounded-2xl shadow-[0_12px_40px_rgba(15,23,42,0.12)] px-8 py-6 md:px-10 md:py-7 text-center max-w-sm w-full">
