            ...
            <section className="px-4 sm:px-6 md:px-12 lg:px-16 pt-2 pb-6 sm:py-8 lg:py-20 max-w-[1400px] mx-auto w-full flex flex-col lg:grid lg:grid-cols-12 gap-3 sm:gap-8 lg:gap-8 items-start lg:items-center">
              ...
              <div className="order-1 w-full lg:order-2 relative h-[clamp(176px,38vh,220px)] sm:h-[340px] lg:h-[640px] xl:h-[700px] rounded-bl-[24px] sm:rounded-bl-[40px] rounded-br-[8px] rounded-tl-[8px] rounded-tr-[24px] sm:rounded-tr-[40px] overflow-hidden lg:overflow-visible lg:col-span-5 z-10 shrink-0 mb-2 sm:mb-14 lg:mb-0">
                ...
                {(slide.feature_title || slide.feature_title_icon) && (
                  <div className="absolute bottom-2 left-2 sm:bottom-4 sm:left-4 lg:-bottom-6 lg:-left-10 bg-white rounded-xl sm:rounded-2xl px-2.5 py-2 sm:p-5 lg:p-6 shadow-[0_8px_30px_rgb(0,0,0,0.08)] flex items-center gap-2 sm:gap-4 border border-slate-100 z-20 w-auto max-w-[min(52%,11rem)] sm:max-w-[280px]">
                    <div className="w-7 h-7 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-orange-50 flex items-center justify-center shrink-0">
                      {slide.feature_title_icon?.url ? (
                        <img src={slide.feature_title_icon.url} className="w-3.5 h-3.5 sm:w-6 sm:h-6 object-contain" alt="" />
                      ) : (
                        <Award className="w-3.5 h-3.5 sm:w-6 sm:h-6 text-orange-500" strokeWidth={2.5} />
                      )}
                    </div>
                    <div className="flex flex-col min-w-0 flex-1">
                      <span className="font-bold text-[#1e40af] text-[10px] sm:text-[15px] leading-snug line-clamp-2">
                        {slide.feature_title}
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 mb-0 sm:mb-14 w-full">
                  {quoteButtonText && quoteButtonLink && (
                    <Link
                      ...
                      className="inline-flex items-center justify-center bg-[#1e40af] hover:bg-blue-800 text-white rounded-xl px-4 sm:px-8 h-10 sm:h-14 shadow-sm text-[13px] sm:text-[16px] font-semibold transition-colors"
                    >
                      {quoteButtonText}
                    </Link>
                  )}
                  {contactNumber && (
                    <a
                      ...
                      className="inline-flex items-center justify-center gap-2 text-[#1e40af] border border-[#1e3a8a]/20 hover:bg-blue-50 rounded-xl px-4 sm:px-8 h-10 sm:h-14 shadow-sm text-[13px] sm:text-[16px] font-semibold bg-white transition-colors"
                    >
                      <Phone className="w-4 h-4 sm:w-5 sm:h-5 text-[#2563EB] shrink-0" />
